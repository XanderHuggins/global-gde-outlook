...
This repository contains all scripts used to preprocess input data, run analyses, and generate the figures presented in the manuscript. These scripts perform tasks such as raster harmonisation and spatial summary statistic calculations across several global datasets.

### Repository structure

- **`on_button.R`** calls `here()` to set the project root and sources all scripts in the folder `0-setup/`
- **`0-setup/`** loads required packages, sets session options (e.g., seed, temp directory), and defines a small set of custom functions
...
Scripts within `1-preprocessing/` and `2-analysis-and-plots/` are numbered in the order they should be run. Some scripts depend on outputs generated by scripts earlier in the numbered order.
