...
# 
# data:  coll_df$aqua_GDE_dens and coll_df$terr_GDE_dens
# t = 191.68, df = 52392, p-value < 2.2e-16
...
landform_lookup = c("1" = "MOUNTAINS", "2" = "HILLS", "3" = "PLATEAUS", "4" = "PLAINS")
...
for (i in 1:4) {
  ...
  ggsave(file = paste0(here("plots"), "/RIBBON_aridity_", landform_lookup[as.character(i)], ".png"),
...
for (i in 1:4) {
  ...
  ggsave(file = paste0(here("plots"), "/RIBBON_seasonality_", landform_lookup[as.character(i)], ".png"),
         plot = last_plot(), device = "png",
         width = 250/4, height = 250/4, units = "mm", dpi = 400)
...
zone_summary = coll_df |>
  ...
  mutate(
    desc = case_when(
      ...
      lifezones == 39 ~ "Tropical rain forest", # some typologies split from 38 but not the case with this HLZ data
      TRUE ~ desc
    )
  )


## lookup table of holdridge life zone and its position within hexbin plot, generated through claude.ai prompt
...
## shape of the hexbin grid, generated through claude.ai prompt
full_grid = tribble(
  ~row, ~col,
  1, 4, 1, 5, 1, 6,
  2, 4, 2, 5, 2, 6, 2, 7,
...
lifezone_df = coll_df |> 
  ...
  mutate(
    desc = case_when(
      ...
      lifezones == 23 ~ "Warm temperate / Subtropical wet forest",
      lifezones == 24 ~ "Warm temperate / Subtropical rain forest",
      lifezones == 39 ~ "Tropical rain forest",
      TRUE ~ desc
    )
  )
...
# Spearman's rank correlation rho
# 
# data:  zone_means$mean_aqua and zone_means$mean_terr
# S = 650, p-value < 2.2e-16
# alternative hypothesis: true rho is not equal to 0
# sample estimates:
#       rho 
# 0.9288762 
...
############################
## Further plotting is below
## but these plots aren't  
## inclded in the manuscript
############################
